...
class DayItinerary(models.Model):
    destination = models.ForeignKey(Destinations, on_delete=models.CASCADE, related_name="itineraries")
    name = models.CharField(max_length=255)
    details = models.TextField()
    created_by = models.ForeignKey('TeamMember', on_delete=models.SET_NULL, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class Itinerary(models.Model):
    name = models.CharField(max_length=255)  # Itinerary name
    ...
    MARKUP_TYPE_CHOICES = [('fixed', 'Fixed Amount (₹)'), ('percentage', 'Percentage (%)')]
    markup_type = models.CharField(max_length=10, choices=MARKUP_TYPE_CHOICES, default='fixed')
    markup_value = models.DecimalField(max_digits=10, decimal_places=2, default=0, help_text="Fixed amount or percentage rate")
    total_individual_markup = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))
    global_markup = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0.00'))

    created_by = models.ForeignKey('TeamMember', on_delete=models.SET_NULL, null=True, blank=True, related_name='created_itineraries', verbose_name="Created By")
    ...
    selected_option = models.CharField(max_length=50, blank=True, null=True, verbose_name="Selected Option")
    ...
    archived_reason = models.CharField(
        ...
        help_text="Why was this itinerary archived (e.g., 'Edited - dates changed', 'User cancelled')"
    )
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        # ✅ NEW: Index for better query performance on status and parent
        indexes = [
    ...
    def is_active(self):
        ...
        return self.status != 'archived' and self.status != 'cancelled'

    def get_child_versions(self):
        """Get all versions created from editing this itinerary"""
        return self.versions.filter(status__in=['draft', 'quoted', 'confirmed', 'active'])

    def get_active_version(self):
        """Get the active/current version of this itinerary (if edited)"""
        if self.parent_itinerary:
            return None  # This is a version, return None
        # Check if this itinerary has any active child versions
        active_child = self.versions.filter(status__in=['draft', 'quoted', 'confirmed']).first()
